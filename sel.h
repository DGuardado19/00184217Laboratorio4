void createLocalA(Matrix &A,mesh m){
    //ELEMENT_LENGTH, TAF, KAPA, LAMDA, NI, PSI, ALFA, SIGMA, PI
    float u_bar = m.getParameter(TAF);
...
void createLocalC(Matrix &C,mesh m){
    //ELEMENT_LENGTH, TAF, KAPA, LAMDA, NI, PSI, ALFA, SIGMA, PI
    float rho = m.getParameter(LAMDA);
...
void createLocalD(Matrix &D,mesh m){
    ...
    D.at(0).at(0) += ni/l;  D.at(0).at(1) +=  -ni/l;
    D.at(1).at(0) +=  -ni/l;  D.at(1).at(1) +=  ni/l;
}
void createLocalE(Matrix &E,mesh m){
    //ELEMENT_LENGTH, TAF, KAPA, LAMDA, NI, PSI, ALFA, SIGMA, PI
    float rho = m.getParameter(ALFA);
    E.at(0).at(0) += -(3*rho)/2;    E.at(0).at(1) += (3*rho)/2;
    E.at(1).at(0) += -(3*rho)/2;    E.at(1).at(1) += (3*rho)/2;
}
void createLocalF(Matrix &F,mesh m){
    //ELEMENT_LENGTH, TAF, KAPA, LAMDA, NI, PSI, ALFA, SIGMA, PI
    float rho = m.getParameter(SIGMA);
    F.at(0).at(0) += -rho/2;    F.at(0).at(1) += rho/2;
    F.at(1).at(0) += -rho/2;    F.at(1).at(1) += rho/2;
}
...
Vector createLocalb(int element,mesh &m){
    ...
    float f = m.getParameter(PSI), l = m.getParameter(ELEMENT_LENGTH),
    pi = m.getParameter(PI);
...
void assemblyK(element e,Matrix localK,Matrix &K,int nnodes){
    ...
    K.at(index3).at(index3) += localK.at(2).at(3);
    K.at(index4).at(index3) += localK.at(3).at(2);
    K.at(index3).at(index4) += localK.at(3).at(3);
